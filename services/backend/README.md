...
1. Не допускается использование WebSocket;
...
#### Transport
Занимается обработкой входящих запросов различных протоколов (REST API, GraphQL, RPC) и изолирует особенности описания каждого протокола. Не допускается наличие блоков бизнес-логики.
...
#### Чем заменить WebSocket?
Необходимо использовать комбинацию HTTP для запросов серверу и SSE для получения уведомлений от сервера.
